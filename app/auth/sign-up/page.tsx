...
import { signUp, signIn, useSession, isGoogleSocialEnabled } from "@/lib/auth-client";
...
// Quick client-side disposable-domain check. Server has the full list.
...
export default function SignUpPage() {
  ...
  // Phone first — rural India audience prefers phone over email.
  ...
  // Phone-mode state — same OTP flow as sign-in (verify auto-creates user)
  ...
  useEffect(() => {
    if (session && !signupComplete) {
      ...
      router.push(user.pregnancyStage ? "/dashboard" : "/onboarding");
    }
  ...
  const handleEmailSignUp = async (e: React.FormEvent) => {
    ...
    if (CLIENT_DISPOSABLE_HINT.test(trimmedEmail)) {
      setError("Please use a real email — disposable addresses aren't supported.");
      return;
    }
    setLoading(true);
    await signUp.email({
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setLoading(false);
      if (!data.ok) {
        setError(data.message ?? "Couldn't send OTP. Use email instead.");
        return;
      }
  ...
  // Email signup success — show "check your inbox" since verification is required
  if (signupComplete) {
    return (
      <div className="flex min-h-screen items-center justify-center p-4">
        ...
        >
          ...
          <h1 className="text-2xl font-heading text-primary mb-2">Check your inbox</h1>
          <p className="text-sm text-muted-foreground leading-relaxed">
            We just sent a verification link to{" "}
            <strong className="text-foreground">{email}</strong>. Click it to activate your account.
          </p>
    ...
    <div className="flex min-h-screen items-center justify-center p-4">
      ...
      >
        <div className="mb-6 text-center flex flex-col items-center">
          <Image src="/NutriLogo.svg" alt="NutriMama Logo" width={56} height={56} className="rounded-2xl mb-3 shadow-lg" />
          <h1 className="text-2xl sm:text-3xl font-heading text-primary">Join NutriMama</h1>
          <p className="mt-1 text-sm text-muted-foreground">Create your account in 30 seconds</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-1 p-1 bg-muted rounded-2xl mb-6">
          <button
            ...
            className={
              ...
              (mode === "phone" ? "bg-card text-primary shadow-sm" : "text-muted-foreground hover:text-foreground")
            }
          >
          ...
          <button
            ...
            className={
              "flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold transition-colors " +
              (mode === "email" ? "bg-card text-primary shadow-sm" : "text-muted-foreground hover:text-foreground")
            }
          >
              ...
              {phase === "enter-phone" ? (
                ...
                <form onSubmit={handleVerifyOtp} className="space-y-4">
                  <div className="flex items-center gap-2 mb-1">
                    <button
                      type="button"
                      onClick={() => { setPhase("enter-phone"); setCode(""); setError(""); }}
              ...
              {googleSocialEnabled && (
                <>
                  <div className="my-5 flex items-center">
                    ...
                    <span className="px-3 text-[10px] font-medium text-muted-foreground uppercase whitespace-nowrap tracking-wider">or</span>
                    <div className="h-px w-full bg-border" />
                  </div>
                  <button
                    onClick={async () => {
